...
namespace FixedWidthParser.Readers
{
    ...
    /// optional culture and a buffer size; reusable across many reads.
    /// <para>
    /// <b>Column offsets are measured in bytes</b> (see <see cref="Utf8FixedWidthParser{TModel}"/>),
    /// which is exact for the single-byte/ASCII payloads typical of flat files. There is no
    /// <see cref="CommunityToolkit.HighPerformance.Buffers.StringPool"/> on this path: string columns
    /// are decoded with <see cref="System.Text.Encoding.UTF8"/> and not interned. A leading UTF-8
    ...
    public sealed class FixedWidthByteReader<TModel> where TModel : new()
    {
        ...
        public FixedWidthByteReader(IFormatProvider? formatProvider = null, int bufferSize = 4096)
        {
        ...
        /// into <paramref name="model"/> using the configured culture. Returns <see langword="false"/>
        /// when the line is too short for the layout or a non-string column fails to parse.
        /// </summary>
        public bool TryParse(ReadOnlySpan<byte> line, out TModel model)
            => _parser.TryParse(line, _formatProvider, out model);
        ...
        public Utf8FixedWidthRecordEnumerable<TModel> Read(Stream stream, bool leaveOpen = false)
        {
            ...
            return new Utf8FixedWidthRecordEnumerable<TModel>(
                _parser, () => stream, ownsStream: !leaveOpen, _formatProvider, _bufferSize);
        }
        ...
        public Utf8FixedWidthRecordEnumerable<TModel> ReadFile(string path)
        {
            ...
            return new Utf8FixedWidthRecordEnumerable<TModel>(
                ...
                ownsStream: true, _formatProvider, _bufferSize);
        }
